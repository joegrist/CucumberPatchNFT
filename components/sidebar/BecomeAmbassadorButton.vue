<template>
	<div>
		<b-button size="sm" block variant="primary" v-b-modal="modalId"
...
export default {
	...
	validations: {
		ambassadorCode: {
			...
			async isUnique(value) {
				...
				const { data: isTaken } = await this.$axios.get('/referrals/exists', {
					params: { code: this.ambassadorCode },
				})
				return !isTaken
			},
		},
	...
	computed: {
		validation() {
			return this.$v.ambassadorCode.$dirty
				? !this.$v.ambassadorCode.$anyError && this.$v.ambassadorCode.isUnique
				: null
		},
	},
	methods: {
